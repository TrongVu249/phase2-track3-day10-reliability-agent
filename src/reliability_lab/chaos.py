        ...
        for entry in breaker.transition_log:
            if entry["to"] == "open":
                ...
                if open_ts is not None:
                    ts_close = entry["ts"]
                    if isinstance(ts_close, (int, float)) and isinstance(open_ts, (int, float)):
                        recovery_times.append((ts_close - open_ts) * 1000)
                    open_ts = None
...
def run_scenario(config: LabConfig, queries: list[str], scenario: ScenarioConfig) -> RunMetrics:
    ...
    # 4. Count circuit_open_count from breaker transition logs
    ...
    # 5. Set recovery_time_ms
    metrics.recovery_time_ms = calculate_recovery_time_ms(gateway)

    # Clean up Redis connection if any
